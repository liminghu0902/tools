...
var _index2 = __webpack_require__(328);

$('#form').submit(function (e) {
	e.preventDefault();
	_index.lf.plugin.validate($(this));
});

console.log(_index2.tools);
...
lf.plugin = function ($) {
	...
	var _plugins = {
		validate: function validate(formName, options) {
...
var tools = function () {
	...
	return {
		...
		//获取取样式
		getStyle: function getStyle(dom, style) {
			if (dom.currentStyle) {
		...
		//跨浏览器绑定事件
		bindEvent: function bindEvent(obj, evt, fn, isbub) {
			if (!obj) {
				return;
			}
		...
		//跨浏览器删除事件
		offEvent: function offEvent(obj, evt, fn) {
			if (!obj) {
				return;
			}
		...
		//返回顶部
		backTop: function backTop(btnId) {
		...
		//复制到剪贴板
		copyTextToClipboard: function copyTextToClipboard(text, success, error) {
			...
			if (_global.clipboardData) {
				...
				var styleArr = ['position:', 'fixed;', 'top:', '0;', 'left:', '0;', 'padding:', '0;', 'width:', '1px;', 'height:', '1px;', 'border:', 'none;', 'outline:', 'none;', 'boxShadow:', 'none;', 'background:', 'transparent'];
		...
		//解析带标签的字符串
		extractText: function extractText(summary) {
			if (!summary) {
				return '';
			}
